...
class SidebarService
{
    ...
    public static function build(array $company, string $currentPath, ?string $routeName = null, string $activeSlug = '', ?array $userPermissions = null): array
    {
        ...
        $secondaryItemsRaw = self::filterByPermissions(self::contextualSecondaryItems($context), $userPermissions);
        ...
        $secondaryNav = [];
        foreach ($secondaryItemsRaw as $item) {
            $url = self::sidebarUrl($companySlug, $item['url']);
            $isActive = self::isContextualItemActive($resolvedRoute, $item);
            $secondaryNav[] = [
                'id' => $item['id'] ?? self::routePrefixFromItem($item),
                'label' => $item['label'],
                'icon' => $item['icon'],
                'url' => $url,
                'is_active' => $isActive,
        ...
        return [
            ...
            'context' => $context,
            'contextLabel' => self::contextLabel($context),
    ...
    private static function buildSafeSidebarFallback(string $companySlug, string $companyName, array $bhStatus, string $hoursJson, string $currentPath): array
    {
        $globalNav = [
            [
                'id' => 'dashboard',
                'label' => 'Início',
                'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M2.25 12L12 3l9.75 9M4.5 9.75v9.75A1.5 1.5 0 006 21h3v-6h6v6h3a1.5 1.5 0 001.5-1.5V9.75" />',
    ...
    /**
     * Main sidebar items — matches StoreDashboardLayout main group (React).
     * 10 entries: Início, Pedidos, Produtos, Categorias, Clientes, Fidelidade,
     * Analytics, Financeiro, WhatsApp, Configurações.
     */
    private static function globalItems(): array
    {
        return [
            ...
            [
                'id' => 'orders',
                'label' => 'Pedidos',
                'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M16.5 6V5.25A2.25 2.25 0 0014.25 3h-4.5A2.25 2.25 0 007.5 5.25V6m9 0H7.5m9 0H18a2.25 2.25 0 012.25 2.25v9.75A3 3 0 0117.25 21H6.75A3 3 0 013.75 18V8.25A2.25 2.25 0 016 6h1.5" />',
                'url' => '/orders',
                'badge' => null,
                'context' => 'orders',
    ...
    /**
     * Secondary sidebar items — "Mais opções" collapsible group in React.
     */
    private static function secondaryItems(): array
    {
        return [
            [
                'id' => 'payment-methods',
            ...
            [
                'id' => 'kds',
                'label' => 'KDS (Cozinha)',
                'icon' => '<circle cx="12" cy="12" r="9"></circle><path stroke-linecap="round" stroke-linejoin="round" d="M9.75 9a2.25 2.25 0 114.15 1.2c-.33.6-.85 1.02-1.37 1.42-.5.38-.98.77-1.2 1.28M12 16h.01" />',
                'url' => '/kds',
                'context' => 'kds',
                'permission' => null,
            ],
    ...
    /**
     * Label do grupo secundário (colapsível) baseado no contexto da rota atual.
     */
    private static function contextLabel(string $context): string
    {
        ...
        return $labels[$context] ?? 'Mais opções';
    }

    /**
     * Itens secundários contextuais — variam conforme o contexto da rota atual.
     * Ex.: ao abrir a página de Produtos, exibe Ingredientes, Cross-Sell, Personalizações, Custos, Embalagens.
     */
    private static function contextualSecondaryItems(string $context): array
    {
        $map = [
            'products' => [
                [
                ...
                [
                    'id' => 'customization-templates',
                    'label' => 'Personalizações',
                    'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M8.25 7.5V6.108c0-1.135.845-2.098 1.976-2.192.373-.03.748-.057 1.123-.08M15.75 18H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08M15.75 18.75v-1.875a3.375 3.375 0 00-3.375-3.375h-1.5a1.125 1.125 0 01-1.125-1.125v-1.5A3.375 3.375 0 006.375 7.5H5.25m11.9-3.664A2.251 2.251 0 0015 2.25h-1.5a2.251 2.251 0 00-2.15 1.586m5.8 0c.065.21.1.433.1.664v.75h-6V4.5c0-.231.035-.454.1-.664M6.75 7.5H4.875c-.621 0-1.125.504-1.125 1.125v12c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V16.5a9 9 0 00-9-9z" />',
                    'url' => '/customization-templates',
                    'permission' => 'view_products',
                ],
                [
                    'id' => 'cross-sell-groups',
                    'label' => 'Cross-Sell',
            ...
            'customers' => [
                [
                    'id' => 'customers-create',
                    'label' => 'Novo Cliente',
                    'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M19 7.5v3m0 0v3m0-3h3m-3 0h-3m-2.25-4.125a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zM4 19.235v-.11a6.375 6.375 0 0112.75 0v.109A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766z" />',
        ...
        // Fallback (dashboard, analytics, payment-methods, delivery-fees, coupons, ifood, kds, etc.):
        // mostra o conjunto antigo de "Mais opções" para preservar acesso rápido.
        return $map[$context] ?? self::secondaryItems();
    }
}
