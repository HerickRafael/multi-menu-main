...
import type { DashboardData, OrdersPerHourPoint, RecentEvent } from '@/types/dashboard'
...
function PlaceholderKpiCard({ title, value, subtitle, icon: Icon, variant }: {
  title: string
  value: string | number
  subtitle?: string
      ...
      {limited.map((point) => {
        ...
        return (
          <div key={`${point.hour}-${point.orders}`} className="flex flex-col items-center gap-1">
            <div className="w-full rounded-sm bg-primary/20" style={{ height: `${height}px` }} title={`${point.hour}: ${point.orders}`} />
  ...
  return (
    <PageContainer>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4">
        ...
        <PlaceholderKpiCard title="Usuários" value={formatNumber(safeData.kpis.users_total)} icon={MessageSquare} variant="success" />
